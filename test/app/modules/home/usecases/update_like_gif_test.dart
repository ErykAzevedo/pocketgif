...
import 'package:pocketmeme/app/modules/home/domain/usecases/update_like_gif.dart';

class BackRepositoryMock extends Mock implements BackRepository {}

main() {
  final repository = BackRepositoryMock();
  final usecase = UpdateLikeGifImpl(repository);
  ...
  test('Informando parametros corretos retorna string', () async {
    when(repository.updateLikeGif(any, any, any)).thenAnswer((_) async => Right('ok'));
  ...
  test('Informando parametros incorretos retorna error', () async {
    when(repository.updateLikeGif(any, any, any)).thenAnswer((_) async => Left(DatasourceError()));
    var result = await usecase('a', 'b', 'c');
